...
void Target::socks_host(string socks_host)
{
    socks_host_ = socks_host;
}

void Target::socks_port(string socks_port)
{
...
string& Target::socks_host()
{
    return socks_host_;
}

string& Target::socks_port()
{
    return socks_port_;
}
...
Session::Session(posix::stream_descriptor out, string query) : out_(move(out)), flag_(true)
{
    ...
    if (socks_host.length() && socks_port.length()) {
        for (auto &i : target_) {
            i.second.socks_host(socks_host);
            i.second.socks_port(socks_port);
        }
    }
}
